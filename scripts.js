...
const existingShape = [[50.15579, -6.28418],[49.83444, -5.41626],[50.00421, -4.80103],[50.10296, -4.44946],[50.11706, -3.65295],[50.50295, -2.59827],[51.08972, -3.25745],[51.40263, -3.87817],[51.2516, -4.72961],[50.73993, -5.36133],[50.47848, -5.87219]]
let snapTo = false;
...
addFreedraw = function () {
	freeDraw = new FreeDraw({ mode: FreeDraw.ALL, mergePolygons: false});
...
freedrawEvent = function(event) {
	if (event.eventType === 'create'){
		...
		freeDraw.clear();
		// coast polyline experiments:
		// const expanded = turf.buffer(L.polyline(event.latLngs).toGeoJSON(), 10, {units: 'kilometers'});
		// const intersect = turf.intersect(expanded, coastCutArea);
		// const drawnLine = turf.lineString(intersect, coastCutArea);
		// L.geoJSON(drawnLine.geometry.coordinates).addTo(map);
	}
}
